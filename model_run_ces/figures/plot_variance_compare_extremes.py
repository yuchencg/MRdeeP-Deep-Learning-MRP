"""
Per-state model variance — three sample sizes side by side, extremes only.

Same layout as plot_variance_compare.py, but restricted to the 3 states with
the highest sample-3 spread and the 3 states with the lowest, so the contrast
between high- and low-disagreement states is easy to read.

Inputs:  model_run_ces/sample{N}_state/outputs/estimates/{outcome}_state_estimates.csv
Outputs: model_run_ces/figures/{outcome}/variance_by_sample_extremes.png
"""
...
N_EXTREME = 3  # states from each end of the sample-3 spread ranking
...
def plot_one(outcome: str, out_path: Path) -> None:
    ...
    fig, axes = plt.subplots(
        nrows=1, ncols=3,
        figsize=(18, max(4, 0.55 * n_states)),
    ...
    axes[0].set_ylabel("3 highest-variance (top) · 3 lowest (bottom)", fontsize=9)
    ...
    # Suptitle
    fig.suptitle(
        f"Per-state model variance across sample sizes — '{outcome}' (extremes only)\n"
...
def main() -> None:
    ...
    for outcome in OUTCOMES:
        print(f"{outcome}:")
        out_path = FIG_DIR / outcome / "variance_by_sample_extremes.png"
        try:
