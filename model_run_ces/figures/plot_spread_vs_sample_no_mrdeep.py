"""
How estimator disagreement shrinks with sample size (excluding MRdeeP).

For each outcome, plots per-state between-model spread (max - min across
the four non-baseline, non-MRdeeP models) across the three sample sizes.
Each state is a thin grey line; the median state and across-state IQR are
overlaid, and the top-N highest-spread states are labelled in color.

Inputs:  model_run_ces/sample{N}_state/outputs/estimates/{outcome}_state_estimates.csv
Outputs: model_run_ces/figures/{outcome}/spread_vs_sample_no_mrdeep.png
"""
...
def plot_one(outcome: str, out_path: Path) -> None:
    ...
    ax.set_title(
        ...
        f"'{outcome}' — spread across the four non-baseline models (excl. MRdeeP), per state",
...
def main() -> None:
    ...
    for outcome in OUTCOMES:
        print(f"{outcome}:")
        out_path = FIG_DIR / outcome / "spread_vs_sample_no_mrdeep.png"
        try:
