...
Same as plot_variance_compare_extremes.py but drops the mrdeep model entirely.
The sample-3 spread ranking is recomputed over the remaining models, so the
3 highest- and 3 lowest-variance states reflect only what is plotted here.

Inputs:  model_run_ces/sample{N}_state/outputs/estimates/{outcome}_state_estimates.csv
Outputs: model_run_ces/figures/{outcome}/variance_by_sample_extremes_no_mrdeep.png
"""
...
def draw_panel(ax, df: pd.DataFrame, state_order: list[str], sample_n: int) -> None:
    ...
    for y, (_, row) in zip(y_positions, df_ord.iterrows()):
        ...
        ax.text(
            (vmin + vmax) / 2, y + 0.30, f"{(vmax - vmin) * 100:.0f} pp",
...
def plot_one(outcome: str, n_extreme: int, out_path: Path) -> None:
    ...
    fig.suptitle(
        ...
        f"Line spans min/max across non-baseline models; labelled in percentage points; "
...
def main() -> None:
    ...
    for outcome in OUTCOMES:
        ...
        for n in EXTREME_COUNTS:
            out_path = (
                FIG_DIR / outcome / f"variance_by_sample_extremes{n}_no_mrdeep.png"
            )
            try:
